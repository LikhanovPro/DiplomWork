...
@Service //Пробно сделал сервисом, для проверки работы аннотации @Autowired
public class PostGetPost extends ApiPostController {
    ...
    public PostGetPost (int offset, int limit, String mode) {
    ...
    public PostGetPost () {
        //Добавил цикл, что бы наглдно видеть, что аннотация @Autowired не работает или работает
        //В общем ловлю ошибку NullPoint
    ...
    public ResponseEntity getPosts (int offset, int limit, String mode) {
        return ResponseEntity.status(HttpStatus.OK).body(new PostGetPost(offset, limit, mode));
    }
